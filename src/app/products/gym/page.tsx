...
export default function GymPage() {
  ...
  useEffect(() => {
    const loadProducts = async () => {
      try {
        ...
        if (response.ok) {
          ...
          console.log('Gym page loaded products:', updatedProducts.length);
        }
        } catch (error) {
    ...
    const handleUpdate = (event?: CustomEvent) => {
      console.log('Gym page: Product update detected', event?.detail);
      loadProducts();
    };
    ...
    window.addEventListener('storage', (e) => {
      if (e.key === 'productUpdated') {
        console.log('Gym page: Storage event detected');
        loadProducts();
      }
    });
  ...
  const refreshProducts = async () => {
    ...
    try {
      ...
      if (response.ok) {
        const updatedProducts = await response.json();
        setAllProducts(updatedProducts);
        console.log('Gym page manually refreshed:', updatedProducts.length);
      }
    } catch (error) {
  ...
  const gymProducts = allProducts.filter(product => product.category === "gym");

  const filteredProducts = gymProducts.filter(product => {
        ...
        <div className="container mx-auto px-4">
          <div className="text-center mb-16">
            ...
            <p className="text-xl text-black/80 max-w-2xl mx-auto">
              Professional fitness gear to elevate your workout routine
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="text-center mb-8">
          <p className="text-gray-600">
            {isLoading ? 'Loading...' : `Showing ${filteredProducts.length} of ${gymProducts.length} gym products`}
          </p>
        </div>
        ...
        {isLoading ? (
          <div className="text-center py-16">
            ...
            <p className="mt-4 text-black">Loading gym products...</p>
          </div>
        ) : (
          /* Products Grid */
